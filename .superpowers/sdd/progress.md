...
## Slice 4E (Corpus Findings: return, shadowing, module lets)
Plan: docs/superpowers/plans/2026-07-13-nymph-codegen-slice4e-corpus-findings.md (commit 4e2f8735, decisions Y1–Y4). Implementation commit: d692a4bf. ONE dynamic Workflow + one post-workflow fix agent. Corpus findings 1–3 CLOSED — their #[ignore]d golden tests are active; finding 4 (impl-Trait call sites) stays ignored/deferred.
- return (Y1): HirStmt::Return; lower_block intercepts Return in any statement position (never a block tail); expression-position return panics loudly. Emit: statement-position Block/If/While/Match now flatten via block_stmt instead of IIFE-wrapping (also removes needless IIFEs generally); an in_iife_subexpr Cell guard (save/restore around the one into_expression site) panics loudly if a return is emitted inside an expression-position construct.
- shadowing (Y2): Lowerer scope stack; same-JS-scope redeclarations (incl. lets shadowing params — params seed the merged function-body scope) rename to name$N ($ illegal in Nymph identifiers, collision-free vs _tN temps); nested-block shadowing keeps source names.
- module lets (Y3): lowered into HirModule, emitted as module consts, topologically ordered (Kahn) by let-deps where a let calling a function inherits the function's transitive let-refs; unbreakable let-to-let cycles panic loudly.
- Review earned its cost AGAIN: round 1 confirmed 3 real bugs (TDZ on shadowed-initializer reads, TDZ on forward-referencing module lets, unbraced-branch return ICE) — fixed in-loop; the in-loop dependency-reorder fix introduced a NEW critical (mutual recursion truncated by the memoized DFS's cycle guard → incomplete dep sets → TDZ from a zero-diagnostic program) which round 2 caught; fixed post-workflow by replacing the memoized DFS with a worklist fixpoint over the call graph (monotonic, converges to cycle unions). Pattern now twice-confirmed: in-loop fixes need fresh-eyes re-review.
- 340 workspace tests green (+1 ignored: finding 4), fmt/clippy clean. SLICE 4E REVIEWED + FIXED.

## Slice 4F (impl-Trait Params at Call Sites)
Plan: docs/superpowers/plans/2026-07-13-nymph-codegen-slice4f-impl-trait-callsites.md (commit 481fe927, decisions Z1–Z4). Implementation commit: 237560c7. ONE dynamic Workflow; review round 1 ZERO findings. Corpus finding 4 CLOSED — the golden corpus now runs with ZERO ignored tests (all 4 findings fixed).
- Fix: fn_type_of extends the call-site subst with a fresh var per parameter-position synthetic Param (idx >= 1<<28, shared Checker::SYNTHETIC_PARAM_BASE + synthetic_params_in walker); covers direct calls AND function-as-value references (same path). Return-position synthetics stay rigid (loud rejection preserved).
- Investigation overturned Z2: there is NO call-site bound-enforcement machinery for declared generics (FuncSig stores no constraints; param_bounds is body-side only; fresh_subst attaches no obligations). Implementer correctly went parity-only ("substitute fresh, enforce nothing" — same as explicit generics) instead of inventing new machinery mid-run.
- KNOWN SOUNDNESS HOLE (pre-existing, both spellings, now documented): `func f<T: Area>(x: T)` called with a NON-implementing argument type-checks with zero diagnostics and crashes at JS runtime (body's bound-resolved method calls emit x.method()). Needs deferred call-site obligations on inference vars — a dedicated slice candidate (nearest diagnostic family: NotImplemented).
- Facts pinned: synthetics are minted PER MENTION (two `impl Iface` params of one interface are independent for free; no syntax can name one twice); impl-Trait in return position already rejects loudly (opaque T-name message; Never-coercion corner noted).
- 347 workspace tests green, 0 skipped. Also: commit 54900058 (haiku agent) named the algorithms at their sites (Kahn + worklist fixpoint, HM unification + occurs check, hash-consing interner).
- SLICE 4F COMPLETE, REVIEWED CLEAN.
